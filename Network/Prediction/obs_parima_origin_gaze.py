...
import json
import argparse
from parima_origin_gaze import build_model
...
def get_data(data, frame_nos, dataset, topic, usernum, fps, milisec, width, height, view_width, view_height):
	VIEW_PATH = '../Viewport/'
	OBJ_PATH = '../Obj_traj/'
	df = pd.read_csv("D:\experiment\PARIMA-\Prediction\gaze"+"\\"+str(usernum)+"\\"+"video.csv")
	gaze_x_list = df.loc[1:,"GazePoint_x"].to_list()
	gaze_y_list = df.loc[1:,"GazePoint_y"].to_list()
	length = len(gaze_x_list)
	for i in range(15):
		gaze_x_list.append("")
		gaze_y_list.append("")
	for i in range(length+15):
		if gaze_x_list[i] == "" and i== 0:
			gaze_x_list[i] = width * 0.5
		if gaze_x_list[i-1] =="" and i != 0:
			gaze_x_list[i] = gaze_x_list[i-1]
		if gaze_y_list[i] == "" and i== 0:
			gaze_y_list[i] = height * 0.5
		if gaze_y_list[i-1] =="" and i != 0:
			gaze_y_list[i] = gaze_y_list[i-1]
	...
	return data, frame_nos, max_frame,total_objects, gaze_x_list, gaze_y_list
...
def main():
	...
	matrix_error = []
	result2 = open("result2.txt",'a')
	

	# Run for all users
	for usernum in range(nusers):
		print('User_{}'.format(usernum+1))

		data, frame_nos = [],[]
		# Get data
		data, frame_nos, max_frame,total_objects, gaze_x, gaze_y = get_data(data, frame_nos, args.dataset, args.topic, usernum+1, args.fps, milisec, width, height, view_width, view_height)
		# Build model
		act_tiles, pred_tiles, chunk_frames, err, x, y = build_model(data, frame_nos, max_frame, total_objects, width, height, nrow_tiles, ncol_tiles, args.fps, pred_nframe, gaze_x, gaze_y)
		...
		avg_x_mae = x
		avg_y_mae = y
		avg_x_mae_all = np.mean(list(zip(*x_mae)), axis=1)
		avg_y_mae_all = np.mean(list(zip(*y_mae)), axis=1)
		avg_manhattan_error = np.mean(list(zip(*manhattan_error)), axis=1)
		print('User_{}'.format(usernum+1),file = result2)
		print('Avg. X_MAE_ALL: {}'.format(avg_x_mae_all[-1]),'Avg. X_MAE_ALL rate: {}'.format(avg_x_mae_all[-1]/width),file = result2)
		print('Avg. Y_MAE_ALL: {}'.format(avg_y_mae_all[-1]),'Avg. Y_MAE_ALL rate: {}'.format(avg_y_mae_all[-1]/height),file = result2)
		print('Avg. X_MAE: {}'.format(avg_x_mae[-1]),'Avg. X_MAE rate: {}'.format(avg_x_mae[-1]/width),file = result2)
		print('Avg. Y_MAE: {}'.format(avg_y_mae[-1]),'Avg. Y_MAE rate: {}'.format(avg_y_mae[-1]/height),file = result2)
		print('Avg. Manhattan Error: {}'.format(avg_manhattan_error[-1]),file=result2)
		print("QoE: {}".format(q))
