...
def pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count, width, height, nrow_tiles, ncol_tiles,gaze_x,gaze_y):
	x_pred, y_pred = 0,0
	series = []
	shift_x = False
	...
	for k in range(len(frames)):
		...
		if(k == 0):
			...
			inp_k["gaze_x"] = gaze_x[k+14]
			inp_k["gaze_y"] = gaze_y[k+14]
...
def build_model(data, frame_nos, max_frame, tot_objects, width, height, nrow_tiles, ncol_tiles, fps, pred_nframe,gaze_x,gaze_y):
	...
	while True:
		...
		metric_X, metric_Y, tile_manhattan_error, count, act_tiles, pred_tiles = pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count, width, height, nrow_tiles, ncol_tiles,gaze_x,gaze_y)
