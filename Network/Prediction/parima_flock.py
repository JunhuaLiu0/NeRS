...
def pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count, width, height, nrow_tiles, ncol_tiles,usernum,topic,quater_npy):
	x_pred, y_pred = 0,0
	series = []
	shift_x = False
	view_width = 2560	
	...
	for i in range(usernum):
		mean = 0
		for j in prev_frames:
			try:
				a = math.sqrt(1-(quater_npy[i][j][4])**2)
				a2 = math.sqrt(1-(quater_npy[usernum-1][j][4])**2)
				mean += ((quater_npy[i][j][1]/a-quater_npy[usernum-1][j][1]/a2)**2+(quater_npy[i][j][2]/a-quater_npy[usernum-1][j][2]/a2)**2+(quater_npy[i][j][3]/a-quater_npy[usernum-1][j][3]/a2)**2)
			except:
				mean += 0
		diff.append(mean/len(prev_frames)*10)
	diff = np.array(diff)
	gamma = 10
	phi = np.quantile(diff, 0.8)
	# w = np.exp(-gamma*(diff-phi))/(1+np.exp(-gamma*(diff-phi)))
	w = np.exp(-gamma*(diff-phi))/(1+np.exp(-gamma*(diff-phi)))
	beta = w/(w.sum())
	...
	for k in range(len(frames)):
		...
		if(k == 0):
			...
			for i in range(usernum):
				view_info = pickle.load(open('../Viewport/ds1/viewport_ds1_topic{}_user{}'.format(topic,i+1), 'rb'), encoding='latin1')
				x_list.append(int(view_info[frames[k]][1][1]*width/view_width))
				y_list.append(int(view_info[frames[k]][1][0]*height/view_height))
			# print(beta)
			# print(x_list)
			flocking_x = int((beta * x_list).sum())
			flocking_y = int((beta * y_list).sum())
			if(shift_x==True):
				x_pred_list[k] = x_pred_list[k] - width
			inp_k['VIEWPORT_x'] = x_pred_list[k-1]
			inp_k['VIEWPORT_y'] = y_pred_list[k-1]
			inp_k["flocking_x"] = flocking_x
			inp_k["flocking_y"] = flocking_y
			print("Flocking_x",flocking_x)
			print("Flocking_y",flocking_y)
...
def build_model(data, frame_nos, max_frame, tot_objects, width, height, nrow_tiles, ncol_tiles, fps, pred_nframe,usernum,topic,quater_npy):
	...
	while True:
		...
		metric_X, metric_Y, tile_manhattan_error, count, act_tiles, pred_tiles = pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count, width, height, nrow_tiles, ncol_tiles,usernum,topic,quater_npy)
