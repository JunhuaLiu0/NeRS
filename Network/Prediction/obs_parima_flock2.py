...
import json
import argparse
from parima_flock2 import build_model
...
def main():
	...
	parser.add_argument('-U', '--usernum', required=False, help='Preferred bitrate quality of the video (360p, 480p, 720p, 1080p, 1440p)')
	...
	for usernum in range(nusers):
		usernum = int(args.usernum)
		print('User_{}'.format(usernum))
		quater_npy = np.load("parse{}.npy".format(args.topic),allow_pickle=True)
		data, frame_nos = [],[]
		# Get data
		...
		# Build model
		act_tiles, pred_tiles, chunk_frames, err, x, y = build_model(data, frame_nos, max_frame, total_objects, width, height, nrow_tiles, ncol_tiles, args.fps, pred_nframe,usernum+1,args.topic,quater_npy)
		...
		result = open("result{}-8.txt".format(args.topic),'a')
		print('User_{}'.format(usernum+1),file = result)
		print('Avg. X_MAE_ALL: {}'.format(avg_x_mae_all[-1]),'Avg. X_MAE_ALL rate: {}'.format(avg_x_mae_all[-1]/width),file = result)
		print('Avg. Y_MAE_ALL: {}'.format(avg_y_mae_all[-1]),'Avg. Y_MAE_ALL rate: {}'.format(avg_y_mae_all[-1]/height),file = result)
		print('Avg. X_MAE: {}'.format(avg_x_mae[-1]),'Avg. X_MAE rate: {}'.format(avg_x_mae[-1]/width),file = result)
		print('Avg. Y_MAE: {}'.format(avg_y_mae[-1]),'Avg. Y_MAE rate: {}'.format(avg_y_mae[-1]/height),file = result)
		print('Avg. Manhattan Error: {}'.format(avg_manhattan_error[-1]),file=result)
		print("\n======= RESULTS ============")
